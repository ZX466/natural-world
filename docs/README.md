...
| M4-P1/P2 建造预算预研 + 定标 | pi | `docs/perf/m4-build-budget-preplan.md` + `sim/tests/bench/test_bench_structure.py` + `thresholds.py` M4-P2 段 | ✅ main `88284c3`（P1）/ ⏳ P2 待收编 | P1：tick 预算盘点（余量 2.10ms=13%）+ 坍塌 BFS 成本模型（BFS 10k=1.27ms vs 逐对象事件 51ms=307% tick）+ 施工推进语义 perf 建议 + 红线草案；P2：实现实测定标（施工推进 100 点 0.175ms→线 **0.30ms** / 坍塌单帧 0.50ms→线 **0.85ms**，均向下修正草案；观察态 `_record_proposal`）|
| M4-P3 意愿/独白热路径 bench | pi | `sim/tests/bench/test_bench_willingness.py` + `thresholds.py` `WILLINGNESS_TICK_LIMIT_MS` + `docs/perf/m4-willingness-hotpath.md` | ⏳ 待收编 | B3 接线补数（13 用例）：expression 纯函数 0.04(band0)/0.4µs(band≥1) per call；B3 热路径 2.1µs(band0)/~165µs(band≥1) per tick；`runtime.tick` 50 NPC 增量 None 0.716→注入 **+0.20ms/tick**（+27.9%，占 16.6ms tick 1.2%）；提案线 0.35ms（只管注入增量，观察态）|
