...
## 8. 裁决（裁 16，2026-09-26 Claude 主树——V1-V4 全采主张）

1. **V1 采**：巧合 lift 事件流名 `mix.adverse`（中性，不泄语义）；`magnitude` 进
   payload（裁 15「dev 日志留痕 OK」先例适用——数值在事件流=开发者可重放，
   Agent 感知面零可见不变）。
2. **V2 采 off**：T5/T2 熵模式 = `off`（YAGNI）。熵注入走事件流本身可重放，
   fixture 重放是第二套机制，M4 不建；若 T5 断言组实测需要再走提案。
3. **V3 采放行**：「老盯着我/净赶在我头上」类被动感受表述放行——§11「只觉得
   今天不顺」的实现面本来就需要这类词；巧合不是命令源不触 §19-四。
   T1 钉子加灰区白名单面（防后续误加 banned）。
4. **V4 采**：`BANNED_WORDS_META` 新增 `概率`/`注定`；「命中/骰」不加
   （战斗语境自然词，误报面大，白名单兜）——词面变更走 CR 纪律（S1 §5 已列）。

**开工放行**：D 批行为链（巧合连锁熵注入）按本预审+裁 16 口径实施，接线点
= ENTROPY_INJECT 事件族 + 感知帧零泄漏断言（V1 口径）。
