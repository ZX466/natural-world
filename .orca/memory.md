...
## ③ opencode（数据 / 数据库域）
- 【2026-09-24 第九轮快照】**M3-D4 已交付**（B3 实施，裁 10 全采 + codex 预审 7 要点）。分支 `ZX466/opencode`（基于 main `4e381c0`）。**0005_m3_knowledge_governance**：knowledge `add_column`×7（subject_npc_id/subject_attr_id/evidence_seq/source_knowledge_id + source_memory/invalidated/invalid_reason）+ 索引×3 + **CHECK×3**（source 取值域 / confidence 值域 / subject 两列成对）——CHECK 是表级约束 SQLite `ADD COLUMN` 表达不了，故用 `op.batch_alter_table`（建新表→拷→换名），列与 CHECK 同批落；downgrade 回 0002 原型，upgrade/downgrade/re-upgrade 实测干净。**knowledge_store.py**：`KnowledgeStore.write_fact`（写入门唯一，X7）/ `invalidate_by_source`（R1 起点）/ `invalidate_by_row` / `supersede_source_memory`（别名）/ `iter_valid` / `get` + `staged_witnessed_row` + `fill_evidence_seq`。**门禁**：pytest 995 passed/55 skipped/0 failed（+20 新钉子）；ruff ok；**pyright 0 error**（基线已清零）；alembic 零漂移（autogenerate 仅 `pass`）。

- 【B3 实施三条纪律留痕】①**继承失效不继承替代**：`_cascade` 沿 `source_knowledge_id` 广度递归，只置 `invalidated=1`+`invalid_reason`，**表内无任何替代指针**（钉子断言 `superseded_by not in cols`，codex 预审①）；已失效行**不覆写 invalid_reason、不重复计数**（幂等）但**仍向下遍历**（其下游可能尚未失效）。②**X7 写入门**：`memory_scan` 抽出 `decide()` 为唯一判梯，`write()`（记忆）与 `scan_fact()`（知识）共用——知识表不可能成为扫描面旁路；改写放行时**落清洗后文本**（`decision.content`），落原文等于绕过 S3。③**evidence_seq 复用 seq_by_index**：`fill_evidence_seq(session, seq_by_index, pending)` 只按 M2-D3 投影缝给的映射回填，**不读 max(seq) 不自行分配**（否则 witnessed 知识会锚到不存在的 seq）。
- 【踩坑：`_cascade` await 种子】`invalidate_by_row` 初版传同步 `lambda s: [row_id]`，而 `_cascade` 里 `await seeds_fn(session)` → `TypeError: object list can't be used in 'await' expression`（钉子 `test_invalidate_by_row_unknown_id_returns_zero` 抓到）。修法：两个种子源都改 async（`_seeds_by_source` / `_seed_single`）。另 `session.execute(update(...))` 的 `Result` 无 `rowcount` 属性（pyright `reportAttributeAccessIssue`）→ 改为先 `select(Knowledge.invalidated)` 判状态再 UPDATE，语义更清楚且免 type-ignore。
- 【踩坑：知识 CHECK 形状】`witnessed ⇒ evidence_seq NOT NULL` 与 `told ⇒ source_knowledge_id NOT NULL` 表达不了 DB CHECK（0005 只有三条可表达的），落**应用层**（`write_fact` 里 `_check_shape` + `_check_evidence_shape`）；`test_persistence_m3.py` 的旧 `test_knowledge_insert` 直插 ORM 不经应用层，故仍绿——**ORM 直插只保低层存储测试，生产必经 `write_fact`**。
- 【2026-09-23 第八轮快照】**M3-D3 已交付**（B3 提案 + C2 重放路径）。分支 `ZX466/opencode`（基于 main `1ae9e54`，已同步 A2 E1）。**B3 = 提案非实施**：新建 `docs/data/knowledge-five-cols-proposal.md`——`knowledge` 表（`0002_m3_reserved` 已建空表，现 7 列无治理）补 **7 列**（`subject_npc_id`/`subject_attr_id`/`evidence_seq`/`source_knowledge_id`/`source_memory`/`invalidated`/`invalid_reason`）+ **3 索引**（`idx_knowledge_source_memory`/`idx_knowledge_source_kid`/`idx_knowledge_subject`）+ `KnowledgeStore.invalidate_by_source|invalidate_by_row`（**继承失效、不继承替代**，沿 `source_knowledge_id` 递归，事务内幂等分支隔离）+ 迁移 `0005_m3_knowledge_governance` 计划 + 4 待裁决点。**未动 models/迁移**（裁决前）。**C2 已实施**：`NpcStore.materialize_matter_replay(matter_ids)` 从 `events` 重放 `matter.*` 折叠重建 `MatterLedger`，与快照路径**共用同一折叠规则** `fold_matter_snapshot`（`_project_matter` 重构为同源）+ `MatterLedger.from_snapshots` 共用构造点。
