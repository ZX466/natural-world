...
**M4-D2a 已交待收编**（事件族 + `0006_m4_structures` + 分支复合身份；见 ③节 2026-09-26 快照与 talking.txt 回执）。下一批 D2b：structure 投影/重放共用单折叠 + build checkpoint 纯函数与逐位相等；后续 D2c 承重图（按帧摊还）/D2d 材料守恒/TILE_CHANGED 派生。
