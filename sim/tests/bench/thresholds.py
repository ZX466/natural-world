...
# --- M3-P1：检索缝预算红线（裁 7 采，2026-09-23）---
...
# 红线值 = pi 提案原值（Claude 裁 7 采，不变）：A4 接线后按实测对账。
...
# 打分链（20 候选 × 2 钩子）上限。外推 0.029ms/NPC + 10x 余量。
RETRIEVAL_SCORE_PER_NPC_LIMIT_MS = 0.30
# 退化哨兵：候选未收缩（600 全量 × 2 钩子）上限。实测 0.862ms + 2.3x；
